...
class MaturityAnalysisRep(models.TransientModel):
	...
	def get_report(self,type):
		...
		self.env.cr.execute("""
			CREATE OR REPLACE view maturity_analysis_book as ("""+self._get_sql(first_date,self.date,self.company_id.id,type)+""")""")
		
		# print('entro')
		# print(self._get_sql(first_date,self.date,self.company_id.id,type))
	...
	def get_excel(self):
		...
		'TD','NRO COMPROBANTE',u'CU BANCO',u'SALDO MN',u'SALDO ME',u'0 - 30',u'31 - 60',u'61 - 90',u'91 - 120',u'121 - 150',u'151 - 180',u'181 - MÁS']
		...
		for line in self.env['maturity.analysis.book'].search([]):
			...
			worksheet.write(x,9,line.cu_banco if line.cu_banco else '',formats['especial1'])
			worksheet.write(x,10,line.saldo_mn if line.saldo_mn else '0.00',formats['numberdos'])
			worksheet.write(x,11,line.saldo_me if line.saldo_me else '0.00',formats['numberdos'])
			worksheet.write(x,12,line.cero_treinta if line.cero_treinta else '0.00',formats['numberdos'])
			worksheet.write(x,13,line.treinta1_sesenta if line.treinta1_sesenta else '0.00',formats['numberdos'])
			worksheet.write(x,14,line.sesenta1_noventa if line.sesenta1_noventa else '0.00',formats['numberdos'])
			worksheet.write(x,15,line.noventa1_ciento20 if line.noventa1_ciento20 else '0.00',formats['numberdos'])
			worksheet.write(x,16,line.ciento21_ciento50 if line.ciento21_ciento50 else '0.00',formats['numberdos'])
			worksheet.write(x,17,line.ciento51_ciento80 if line.ciento51_ciento80 else '0.00',formats['numberdos'])
			worksheet.write(x,18,line.ciento81_mas if line.ciento81_mas else '0.00',formats['numberdos'])
			x += 1

		widths = [10,12,8,8,6,11,40,6,21,10,16,16,15,15,15,15,15,15,15]
	...
	def getPdf(self):
		...
		for c,fila in enumerate(self.env['maturity.analysis.book'].search([])):
			...
			datos[c+1].append(Paragraph((fila['cu_banco']) if fila['cu_banco'] else '',style_left))
		...
		table_datos = Table(datos, colWidths=[3.5*cm,3.5*cm,3.5*cm,2.5*cm,1.5*cm,4*cm,15*cm,1.2*cm,6*cm,4*cm,3.75*cm,3.75*cm,2.75*cm,2.75*cm,2.75*cm,2.75*cm,2.75*cm,2.75*cm,3*cm], rowHeights = len(datos)*[1*cm])
	...
	def _get_sql(self,first_date,end_date,company_id,type):
		...
		sql = """
			SELECT *
					...
					(SELECT row_number() OVER () AS id,
						...
						FROM get_maturity_analysis('%s','%s',%s,'%s'))T1
								
							LEFT JOIN(	
								SELECT nro_letra,cu_banco
									FROM
									account_letras_payment_manual
									) T2 ON T1.nro_comprobante = T2.nro_letra
