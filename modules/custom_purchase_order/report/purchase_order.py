...
class PurchaseOrder(models.Model):
	...
	def build_report_pdf(self):
		...
		def header_table2(c,pos):
			...
			data=[[Paragraph(u"Ciudad:",p1_1),Paragraph(u"%s"%(self.partner_id.state_id.name or ' '),p2)],]
			hTable=Table(data,colWidths=170,rowHeights=(15))
			hTable.setStyle(TableStyle([
				('VALIGN',(0,0),(-1,-1),'MIDDLE'),
				('BOX',(0,0),(-1,-1),1,colors.black)
		...
		def header_table4(c,pos):
			...
			Table_lines.wrapOn(c,120,500)
			Table_lines.drawOn(c,pos_left,(pos-200)-55)
			...
			c.setFont("Calibri", 10)
			c.drawString(260,(pos-200)-110, str(partner_confirm or ''))
			c.setFont("Calibri-Bold", 10)
			c.drawString(260,(pos-200)-130, "APROBADO")

			c.setFont("Calibri", 10)
			c.drawString(420,(pos-200)-110, str(self.partner_id.name))
			c.setFont("Calibri-Bold", 10)
