...
col_widths0 = [float(i)/100*wUtil for i in ((10,20,11,11) + 4*(12,))] # size of columns (list sum = 100%)
...
p2 = ParagraphStyle('p2',alignment=TA_LEFT,fontSize=8,fontName="Calibri")
p2_1 = ParagraphStyle('p2_1',alignment=TA_CENTER,fontSize=8,fontName="Calibri")
p2_2 = ParagraphStyle('p2_2',alignment=TA_LEFT,fontSize=8,fontName="Calibri")
p3 = ParagraphStyle('p3',alignment=TA_LEFT,fontSize=10,fontName="Calibri")
p4 = ParagraphStyle('p4',alignment=TA_RIGHT,fontSize=8,fontName="Calibri")
...
class SaleOrder(models.Model):
	...
	def build_report_pdf(self):
		...
		total_discount = 0.0
		total_bruto = 0.0
		fmt_dec = lambda float_n: ' {:,.2f}'.format(Decimal("%0.2f" % float_n or 0.0))

		for line in self.order_line:
			codigo_producto   = str(line.product_id.default_code or ' ')
			producto   = str(line.product_id.name or ' ')
			cantidad   = fmt_dec(line.product_uom_qty)
			precio   = fmt_dec(line.price_unit)
			descuento   =  str( '% ' + fmt_dec(line.discount))
			suntotal   = fmt_dec(line.price_subtotal)


			iva   =  str('% '+fmt_dec(line.tax_id.amount))
			amount_b = line.tax_id.compute_all(line.price_unit, line.order_id.currency_id, line.product_uom_qty, product=line.product_id, partner=line.order_id.partner_shipping_id)['total_excluded']# subtotal sin descuento

			total_discount += amount_b - line.price_subtotal if line.discount > 0.0 else 0.0
			total_bruto += amount_b

			data = [[
				Paragraph(codigo_producto,p2_2),
				Paragraph(producto,p2_2),
				Paragraph(cantidad,p4),
				Paragraph(line.product_uom.name,p2_2),
				Paragraph(precio,p4),
				Paragraph(descuento,p4),
				Paragraph(suntotal,p4),
				Paragraph(iva,p4)],]
		...
		def header_table4(c,pos):
			...
			fmt_dec_curr = lambda float_n: self.currency_id.symbol + ' {:,.2f}'.format(Decimal("%0.2f" % float_n or 0.0))

			data = [
					[
					Paragraph(fmt_dec_curr(total_bruto),p2_1), 
					Paragraph(fmt_dec_curr(total_discount), p2_1),
					Paragraph( fmt_dec_curr(self.amount_untaxed),p2_1),
					Paragraph(fmt_dec_curr(self.amount_tax) ,p2_1),
					Paragraph(fmt_dec_curr(self.amount_total), p2_1)
					],
				]
...
#------------------------------------------------------------------------
	...
	# ========
	# WTF ????
	# FIXME TODO Refactorizar ésta huevada, odoo ya tiene métodos para hacer ésto!!!!
